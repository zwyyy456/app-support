...
export function automationDemo(locale, compact = false) {
  ...
  return `<div class="automation-demo ${compact ? 'compact' : ''}" data-automation>
    <div class="flow-demo-heading"><span class="eyebrow">${t('FROM PULL REQUEST TO PROJECT','从 PULL REQUEST 到 PROJECT')}</span><span class="demo-pill">${t('Choose a PR stage','选择 PR 阶段')}</span></div>
    <div class="flow-stages" role="group" aria-label="${t('Choose a pull request stage to see the matching issue status','选择 PR 阶段，查看对应的 Issue 状态')}">
      <button type="button" data-stage="draft" data-map-result="progress" aria-pressed="true"><span class="flow-stage-source">${icon('pull')}<small>PR</small><strong>Draft</strong></span><span class="flow-stage-arrow">${icon('arrow')}</span><span class="flow-stage-target" data-state="progress"><small>Issue</small><strong>In Progress</strong></span></button>
      <button type="button" data-stage="ready" data-map-result="review" aria-pressed="false"><span class="flow-stage-source">${icon('pull')}<small>PR</small><strong>Ready</strong></span><span class="flow-stage-arrow">${icon('arrow')}</span><span class="flow-stage-target" data-ready-target data-state="review"><small>Issue</small><strong data-ready-result-label>In review</strong></span></button>
      <button type="button" data-stage="merged" data-map-result="done" aria-pressed="false"><span class="flow-stage-source">${icon('merge')}<small>PR</small><strong>Merged</strong></span><span class="flow-stage-arrow">${icon('arrow')}</span><span class="flow-stage-target" data-state="done"><small>Issue</small><strong>Done</strong></span></button>
    </div>
    <div class="flow-transfer"><div class="pull-card"><span class="mini-label">PULL REQUEST <span class="mono">#58</span></span><h3>${t('Make room for focused work','给专注的工作留一点空间')}</h3><span class="code-token">Closes #42</span><span class="pull-state" data-pr-state>Draft</span></div><div class="connector" aria-hidden="true"><span></span>${icon('arrow')}<span></span></div><div class="project-result" data-result="progress"><span class="mini-label">PROJECT ITEM <span class="mono">#42</span></span><h3>${t('Make room for focused work','给专注的工作留一点空间')}</h3><span class="result-status" aria-live="polite"><i class="status-dot progress" data-result-dot></i><strong data-result-label>In Progress</strong></span><span class="result-note">${t('Issue status changes with the closing pull request','Issue 状态随关闭关联 PR 一起变化')}</span></div></div>
